...
class FavoriteButton extends Component {
  ...
  getFavoriteStatus(trackid){
    ...
    return new Promise(resolve => {
      self.userid = JSON.parse(sessionStorage.getItem('userDetails'));
      // Get the user details from database
      axios.get(`${originURL}user/isFavorite/${self.userid}/${trackid}`)
      .then(isFavorite => {
        self.setState({ isFavorite: isFavorite.data.isFavorite, isLoading: false });
  ...
  fetchData = async () => {
    ...
    console.log(`Entered <FavoriteButton> componentDidMount(), fetching userid: ${this.userid}`);
    try{
  ...
  favoriteToggle(){
    ...
    if(this.state.isFavorite){
      return new Promise( resolve => {
        axios.put(`${originURL}user/removeFavoriteTrack/${userDetails._id}`, { trackid: self.state.trackid })
        .then(favoritedTrack => {
          self.setState({ isFavorite: false });
          NotificationManager.info('Track removed from favorites.');
          console.log(`Track ${self.state.trackid} removed from favorites.`)
    ...
    // User want add track to favorites list
      ...
      return new Promise( resolve => {
        axios.put(`${originURL}user/addFavoriteTrack/${userDetails._id}`, { trackid: self.state.trackid })
        .then(unFavoritedTrack => {
          self.setState({ isFavorite: true });
          NotificationManager.success('Track added to favorites.');
          console.log(`Track ${self.state.trackid} added to favorites.`)
