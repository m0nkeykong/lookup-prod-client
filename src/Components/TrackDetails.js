...
class TrackDetails extends Component {
        ...
        fetch(getTrackByIdURL(trackId))
            ...
            .then(data => {
                ...
                self.addTrack(
                    data.track._id,
                    data.track.title,
                    data.track.travelMode,
                    data.track.difficultyLevel.star,
                    data.reports,
                    data.userDetails,
                    data.startPoint,
                    data.endPoint,
                    data.track.wayPoints,
                    data.track.description,
                    data.track.disabledTime,
                    data.track.nonDisabledTime
                )
            })
    }
    ...
    getReports(reports, userDetails) {
        ...
        if (reports.length !== 0) {
                    ...
                    <ul className="media-list">
                        <li className="media">
                            <img
                                ...
                                src={userDetails[i].profilePicture}
                                alt="profile"
                            />
                            <div className="media-body">
                                <div className="well well-lg">
                                    <h5 className="media-heading text-uppercase nameTitle">
                                        {userDetails[i].name}
                                    </h5>
            ...
            html.push(<p className="media-report" />)
        }
        return html
    }
    ...
    getWayPoints(wayPoints) {
        ...
        if (wayPoints.length !== 0) {
            for (let i = 0; i < wayPoints.length; i++) {
                ...
                html.push(
                    <p> &#8227; &#9;longitude: {wayPoints[i].longitude}</p>
                )
            }
        }
        return html
    ...
    getStarsForDifficultyLevel(diffLever) {
        ...
        for (let i = 0; i < limitOfStars; i++) {
            ...
            else html.push(<span key={'starKey2'} className="fa fa-star" />)
        }
        return html
    }
    ...
    buildTrack(track) {
        const trackObj = {
            ...
            difficultyLevel:
                track.difficultyLevel !== '' ? track.difficultyLevel : {},
            ...
            nonDisabledTime:
                track.nonDisabledTime !== '' ? track.nonDisabledTime : {},
            ...
            <div key={'container' + i}>
                <div className="col-12 px-4">
                    ...
                    >
                        ...
                        <p className="typeTrack">
                            {this.getIconType(track.travelMode)}
                        </p>
                        <p className="typeTrack">
                            {this.getTimeOfTrack(
                                track.disabledTime,
                                track.nonDisabledTime
                            )}
                        </p>
